...
if isempty(min1)
    min1=ceil((max1(2:end)+max1(1:end-1))/2);
end
...
% column 3, cycle number in the forward.

fprintf('\n');
disp("Pre-cutting for matrix done. Please check N_forward for the squareness manually.");

% check the oddness in the N_forward matrix and give a warning.
AA_warning=find(N_forward(:,3)~=N);
...
for i=1:length(AA_warning)
    ...
    if j==1
        if N_forward(1,3)>N
            N_forward(1,1)=N_forward(1,2)-N+1;
            N_forward(1,3)=N;
    ...
    if j>1&&(N_forward(j,3)==N-1)&&(N_backward(j-1,3)==N+1)
        N_forward(j,1)=N_forward(j,1)-1;
        N_forward(j,3)=N_forward(j,3)+1;
...
% advise to open the N_forward matrix and manually adjust it to square.
close all
% plot function to see cutting situation.
i=48;        % the forward cycle to be checked.
dN=5;       % larger range to show the misalignment of the cutting.
N_forward(i,3)=N_forward(i,2)-N_forward(i,1)+1;
N1=min0(N_forward(i,1)-dN);
N2=max0(N_forward(i,2)+dN);
figure;
plot(auxin1(N1:N2));
hold on;
yyaxis right;
...
for j=N_forward(i,1):N_forward(i,2)
    label=sprintf('%d',j-N_forward(i,1)+1);
...
i_add=N_forward(i,1)+32;      % the number here is the line number before the missing line in the plot.
m_base=N1;
min_add=184624+m_base;
max_add=185780+m_base;
...
% add 1 to everything in N_forward after the added line.
N_forward(i,2)=N_forward(i,2)+1;
N_forward(i,3)=N_forward(i,3)+1;
N_backward(i,2)=N_backward(i,2)+1;
N_backward(i,3)=N_backward(i,3)+1;
for j=i+1:length(N_forward(:,1))
    N_forward(j,1)=N_forward(j,1)+1;
    N_forward(j,2)=N_forward(j,2)+1;
    N_backward(i,2)=N_backward(i,2)+1;
    N_backward(i,3)=N_backward(i,3)+1;
end
...
%% Run the position divison part again to make sure there is no misalignment for backward scan.
...
% a pre-fix for the N_forward, align the lines to the end.
lines=size(N_forward,1);
if lines<N_forward(1,3)
    N_forward=[nan(N_forward(1,3)-lines,3);N_forward];
end

fprintf('\n');
auxin2_forward=matrix_cut_step(min0,max0,auxin2,N_forward);
auxin1_forward=matrix_cut_step(min0,max0,auxin1,N_forward);
r_forward=matrix_cut_step(min0,max0,r,N_forward);
phase_forward=matrix_cut_step(min0,max0,phase,N_forward);
%z_forward=matrix_cut_step(min0,max0,z,N_forward);
...
auxin1_avg=step_3D_average(auxin1_forward,2,z_resolution);

r_avg_bin1=step_3D_average(r_forward,3,auxin1_avg.position);
phase_avg_bin1=step_3D_average(phase_forward,3,auxin1_avg.position);
z_avg_bin1=step_3D_average(auxin1_forward,3,auxin1_avg.position);
...
% before averaging part. all the forward, para and path.
path1=[path(1:end-4),'_processed_before_avg.mat'];
save(path1,"auxin1_forward","auxin2_forward","r_forward","phase_forward","para","path","N_forward","N_backward","missing_line_flag","min0","max0","min1","max1",'-mat');

% after averaging part, all the avg and m data.
path1=[path(1:end-4),'_processed_after_avg.mat'];
...
lines=sum(isnan(N_forward),'all')/3;
N=size(N_forward,1);
...
disp('<a href="matlab:opentoline(''force_curve_3D_5_channels_stepZ_for_missing_lines.m'',670)">Next code</a>');
beep;
...
disp('<a href="matlab:opentoline(''force_curve_3D_5_channels_stepZ_for_missing_lines.m'',770)">Next code</a>');
beep;
...
for i=1:size(r_avg_m_corr,1)
    for j=1:size(r_avg_m_corr,1)
        ...
        try
            ...
            warning('\nPosition: x = %d, y = %d is wrong No value available.\n',i,j);
            F0=[];
            E0=[];
            F0_extended=[];
            E0_extended=[];
            ...
            warningN=warningN+1;
            if (warningN>20)
                warning('\nToo many force calculation error, check if the force calculation function is in the Matlab folder.\n');
            end
        end
...
% matrices.
path1=[path(1:end-4),'_r_phase_z+E_F.mat'];
...
for i=1:size(s0,1)
    s1(i,:)=s0(i,:)-polyval(polyfit(x,s0(i,:),1),x);
end
...
disp('force_curve_3D_5_channels_stepZ_for_missing_lines.m is complete, move on to retrace.');
beep;
